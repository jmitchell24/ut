...
namespace ut
{
    ...
    public:
        ...
        M_DECL_PURE vector_type inverse() const { return vector_type(one_ / x, one_ / y, one_ / z); }
        ...
        ENABLE_IF_FLOAT M_DECL_PURE bool isFltNan   () const { return std::isnan   (x) || std::isnan   (y) || std::isnan   (z); }
        ENABLE_IF_FLOAT M_DECL_PURE bool isFltInf   () const { return std::isinf   (x) || std::isinf   (y) || std::isinf   (z); }
        ENABLE_IF_FLOAT M_DECL_PURE bool isFltNormal() const { return std::isnormal(x) && std::isnormal(y) && std::isnormal(z); }
